...
const HeaderToggle = (props) => {
  ...
  const { selectValue, deliveryVal, pickupVal } = useSelector(
    (state) => state?.menu
  );
  const initalValue =
  ...
  const handleToggleChange = () => {
    ...
    if (deliveryVal == 0) {
      toast.error("This restaurant does not provide delivery service");
      return;
    }

    if (pickupVal == 0) {
      toast.error("This restaurant does not provide pickup service");
      return;
    }
    ...
    <div>
      ...
      >
        ...
        <span
          ...
          className="lg:px-4 px-2 py-2 flex items-center justify-center mt-2 rounded-r-md lg:text-lg text-sm bg-gray-300 lg:h-8 h-6 peer-checked:text-white peer-checked:bg-black"
        >
          {props.value2}
        </span>
      </label>
