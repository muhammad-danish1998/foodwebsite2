...
const CartInc = () => {
  ...
  return (
    <div className="overflow-y-auto max-h-[calc(100vh-30rem)] scroll-smooth cart-scroll">
      <div>
        {/* {console.log(cartlistItemm, "cli ")} */}

        {cartlistItemm?.items?.map((cart, index) => (
          //  setVal(cart?.price),
          <>
            <div
              className="card-list-uper flex justify-between p-6"
              key={index}
            >
            ...
            <div className="card-list-uper flex-row justify-between px-5">
              {cart.multiaddonsarr &&
                cart.multiaddonsarr?.map((addon, index) => (
                  <div className="flex-row flex justify-between">
                    <div>
                      <strong className="text-sm">{index + 1}.</strong>{" "}
                      {addon.multiaddonname}
                    </div>{" "}
                    <div>€{addon.mutliaddonprice}</div>
                  </div>
                ))}
              {/* .filter( (obj, index, self) => index === self.findIndex((t) =>
              t.id === obj.id) ) */}
              {cart.addonarr?.length > 0 &&
                cart.addonarr.map((addon, index) => {
                  return (
                    <div key={index} className="flex-row flex justify-between">
                      <div>
                        <strong className="text-sm">{index + 1}.</strong>{" "}
                        {addon.addonname}
                      </div>
            ...
            <div className="card-list-uper flex justify-between p-2">
              <div className=" ipadAir:pr-4 galxyFold:pr-4 p-2">
                ...
                <input
                  ...
                  class="block w-full  text-gray-900 border border-gray-300 bg-gray-300 sm:text-xs   "
                />
                <button
                  className="lg:p-2 block  lg:text-md  font-bold text-gray-500"
                  onClick={() => handleUpdateNote(cart?.temp, cart?.quantity)}
                >
                  Update
                </button>
              </div>
